...
class Os_model extends CI_Model {
    ...
    public function autoCompleteProduto($q){
        ...
        if($query->num_rows > 0){
            foreach ($query->result_array() as $row){
                $row_set[] = array('label'=>$row['descricao'].' | Precio: S/. '.$row['precoCompra'].' | Stock: '.$row['estoque'],'estoque'=>$row['estoque'],'id'=>$row['idProdutos'],'preco'=>$row['precoCompra']);
            }
    ...
    public function autoCompleteCliente($q){
        ...
        if($query->num_rows > 0){
            foreach ($query->result_array() as $row){
                $row_set[] = array('label'=>$row['nomeCliente'].' | RUC: '.$row['documento'],'id'=>$row['idClientes']);
            }
            echo json_encode($row_set);
        }
    }
    ...
    public function autoCompleteServico($q){
        ...
        if($query->num_rows > 0){
            foreach ($query->result_array() as $row){
                $row_set[] = array('label'=>$row['nome'].' | Precio: S/. '.$row['preco'],'id'=>$row['idServicos'],'preco'=>$row['preco']);
            }
            echo json_encode($row_set);
        }
    }
