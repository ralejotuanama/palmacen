...
class Os extends CI_Controller {
	...
	function gerenciar(){
		...
		$this->data['results'] = $this->os_model->get('os','idOs,dataInicial,dataFinal,garantia,descricaoProduto,defeito,status,observacoes,laudoTecnico,modelo,marca,serie,tipo,tipodocumento','',$config['per_page'],$this->uri->segment(3));
    ...
    function adicionar(){
        ...
        if ($this->form_validation->run('os') == false) {
           ...
           $this->session->set_flashdata('success','La Guía se agregó correctamente'); 
                mail('tuemail', 'Nueva O.S', 'Nueva Orden de Servicio Ingresada');
                redirect('os/editar/'.$id);
    ...
    function editar() {
        ...
        if ($this->form_validation->run('os') == false) {
            ...
            if ($this->os_model->edit('os', $data, 'idOs', $this->input->post('idOs')) == TRUE) {
                ...
                $this->session->set_flashdata('success','Guía editada con éxito!');
